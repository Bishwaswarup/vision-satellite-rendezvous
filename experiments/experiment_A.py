...
MUTED = '#555555'
GRID  = '#DDDDDD'
C1    = '#111111'   # position error line
C2    = '#555555'   # velocity error line
C3    = '#222222'   # convergence pos
C4    = '#666666'   # convergence vel

def style_ax(ax, ylabel, xlabel=None):
...
# ── Fig 1 — time-history ──────────────────────────────────────────────────────
fig, (ax0, ax1) = plt.subplots(2, 1, figsize=(9, 6), facecolor=BG)
fig.subplots_adjust(hspace=0.45, left=0.12, right=0.96, top=0.92, bottom=0.10)

orbit_kw = dict(color=MUTED, linewidth=0.7, linestyle='--', alpha=0.6)

style_ax(ax0, 'Position error  [nm]')
ax0.set_xlim(0, t_hours[-1])
ax0.set_title('Position error', color=TEXT, fontsize=11, pad=6)
for k in range(1, N_ORBITS + 1):
    ax0.axvline(k * T_ORB / 3600, **orbit_kw)
ax0.plot(t_hours, pos_err * 1e9, color=C1, linewidth=1.5)

style_ax(ax1, 'Velocity error  [nm s⁻¹]', xlabel='Time  [h]')
ax1.set_xlim(0, t_hours[-1])
ax1.set_title('Velocity error', color=TEXT, fontsize=11, pad=6)
for k in range(1, N_ORBITS + 1):
    ax1.axvline(k * T_ORB / 3600, **orbit_kw)
ax1.plot(t_hours, vel_err * 1e9, color=C2, linewidth=1.5, linestyle='--')

fig.suptitle('Analytical STM vs DOP853  (rtol = 1×10⁻¹², atol = 1×10⁻¹⁴)',
...
# ── Tolerance convergence sweep ───────────────────────────────────────────────
print("\n=== Tolerance Convergence Tests ===")
print(f"{'rtol':<12} | {'atol':<12} | {'Max Pos Error (m)':<18} | {'Max Vel Error (m/s)'}")
print("-" * 68)
...
# ── Fig 2 — tolerance convergence ────────────────────────────────────────────
fig2, (ax_pos, ax_vel) = plt.subplots(2, 1, figsize=(8, 7), facecolor=BG)
fig2.subplots_adjust(hspace=0.45, left=0.15, right=0.92, top=0.90, bottom=0.10)

style_ax(ax_pos, 'Max position error  [m]')
ax_pos.loglog(rtols, max_pos_errors, marker='o', markersize=6, color=C3,
              linewidth=1.8, markeredgecolor=BG, markeredgewidth=1.2,
              markerfacecolor=C3)
ax_pos.invert_xaxis()   # tight tolerance (small rtol) on left, loose on right
ax_pos.set_title('Position error convergence', color=TEXT, fontsize=11, pad=6)
ax_pos.grid(True, which='minor', color=GRID, linewidth=0.4, alpha=0.8)

style_ax(ax_vel, 'Max velocity error  [m/s]', xlabel='Relative tolerance  (rtol)')
ax_vel.loglog(rtols, max_vel_errors, marker='s', markersize=6, color=C4,
              linewidth=1.8, linestyle='--',
              markeredgecolor=BG, markeredgewidth=1.2,
              markerfacecolor=C4)
ax_vel.invert_xaxis()   # tight tolerance (small rtol) on left, loose on right
ax_vel.set_title('Velocity error convergence', color=TEXT, fontsize=11, pad=6)
ax_vel.grid(True, which='minor', color=GRID, linewidth=0.4, alpha=0.8)
...
fig2.savefig(out_path_conv, dpi=180, bbox_inches='tight', facecolor=BG)
plt.close(fig2)
print(f"\nSaved → {out_path_conv}")
