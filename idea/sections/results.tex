...
The architecture is evaluated in six experiments. Experiments~A--C verify the
individual subsystems against independent references; D and E compare design
choices within the estimator and the controller; F closes the loop and
characterises the operating envelope. Every number in this section is produced
by the scripts named in Sec.~\ref{sec:method} and is reproducible from the
stated seeds.
...
position discrepancy was $4.55\times10^{-10}$~m, i.e.\ below one nanometre,
and the maximum velocity discrepancy was $5.38\times10^{-13}$~m/s
(Fig.~\ref{fig:dynamics_validation}).

We quote these figures to two significant digits deliberately. The comparison
contains no random element, but the DOP853 step sequence is chosen adaptively
and therefore depends on the SciPy release; across the versions tested the
result moves by a few per cent. The physically meaningful statement is that
the numerical error is sub-nanometre, nine orders of magnitude below the
metre-level separations of interest and eight below the centimetre-level pose
errors of Sec.~\ref{sec:res_pose}. Numerical propagation error is therefore
not a contributor to any subsequent result.

Table~\ref{tab:tolerance} and Fig.~\ref{fig:tolerance_convergence} show the
error decreasing monotonically with tolerance, confirming that the integrator
is converging rather than merely reporting small residuals.

\begin{table}[t]
\caption{DOP853 convergence. Maximum error against the analytical HCW STM
over three orbits.}
\label{tab:tolerance}
\centering
\begin{tabular}{llll}
\hline
rtol & atol & Max position [m] & Max velocity [m/s] \\
\hline
$10^{-6}$  & $10^{-8}$  & $4.50\times10^{-4}$  & $4.71\times10^{-7}$ \\
$10^{-8}$  & $10^{-10}$ & $5.18\times10^{-6}$  & $5.66\times10^{-9}$ \\
$10^{-10}$ & $10^{-12}$ & $4.97\times10^{-8}$  & $5.72\times10^{-11}$ \\
$10^{-12}$ & $10^{-14}$ & $4.55\times10^{-10}$ & $5.38\times10^{-13}$ \\
$10^{-13}$ & $10^{-15}$ & $4.73\times10^{-11}$ & $5.14\times10^{-14}$ \\
\hline
\end{tabular}
\end{table}
...
\begin{figure}[t]
    ...
    \caption{Position and velocity differences between the analytical HCW
    solution and DOP853 numerical propagation over three orbital periods.}
...
\begin{figure}[t]
    ...
    \caption{Maximum position and velocity error against relative integration
    tolerance.}
...
Pose sensitivity to image-plane noise was measured over 200 Monte Carlo trials
per noise level, with the target at a fixed 20~m range and attitude, seed 42.
Results appear in Table~\ref{tab:pose_noise} and Fig.~\ref{fig:pose_noise}.
...
Both errors grow essentially linearly with $\sigma_{px}$, as expected when the
geometry is fixed and the noise is small enough for the linearisation about the
true pose to hold. At 20~m and $f = 800$~px a 1~px displacement subtends
2.5~cm, and the observed 4.38~cm translation RMSE at $\sigma_{px} = 1$~px is
consistent with that scale given 15 keypoints spanning a
$372 \times 112$~px image patch.

\paragraph{On the benefit of nonlinear refinement.}
Levenberg--Marquardt refinement reduces translation error by 9--14\% and
attitude error by 10--14\% over the range tested. This is a modest
improvement and we report it as such, because a much larger apparent benefit
is easy to obtain accidentally. The closed-form stage of EPnP is accurate only
if the control-point coefficients $\beta_k$ are themselves refined against the
...
four, which then makes the subsequent LM stage appear to deliver a large gain.
With the reference algorithm implemented in full, most of the achievable
accuracy is present before LM runs.
...
OpenCV EPnP implementation does not handle, and one that arises in
approximately 3\% of random six-point samples drawn from this target's
keypoint set.
...
Table~\ref{tab:ransac} reports the effect of gross correspondence errors, with
a fraction $f_{out}$ of the 15 keypoints replaced by uniformly distributed
image points. Inlier noise is $\sigma_{px} = 1$~px; RANSAC uses a 2~px inlier
threshold, a six-point minimal sample and a 500-iteration budget. A trial
counts as failed if the solver reports failure or the recovered rotation error
exceeds $15^\circ$.
...
Two observations follow. First, a single gross outlier destroys the
least-squares solution: at $f_{out} = 5\%$, fewer than one outlier per frame on
average, bare EPnP fails in 99\% of trials. Outlier rejection is not a
refinement but a precondition for applying a PnP solver to real
correspondences.

Second, RANSAC--EPnP fails in no trial up to $f_{out} = 30\%$, and degrades
only mildly --- 5.18 to 6.07~cm in translation, 0.854 to 1.037$^\circ$ in
attitude. The residual growth is consistent with the reduced number of true
inliers available to the final refit rather than with any breakdown of the
consensus search.
...
sequence of direct pose measurements corrupted by $\sigma_{pos} = 0.5$~m and
$\sigma_{att} = 0.05$~rad, over 300 steps at $\Delta t = 10$~s, from an
initial position offset of 3.9~m and an initial attitude error of $17^\circ$.
This is not an end-to-end camera simulation: the perception chain is
deliberately replaced by an equivalent noise model so that filter behaviour
can be examined without confounding it with pose-solver failures.
...
The two filters are indistinguishable in translation, and the reason is
structural rather than empirical: HCW relative translation is exactly linear,
so the unscented transform and the analytic Jacobian describe the same map and
must agree to machine precision whatever the sigma-point spread. Any
difference can appear only through the attitude and rate block, and there the
UKF is marginally worse in this scenario.

The UKF is also marginally \emph{slower} --- 1.662~s against 1.602~s, 3.7\%
--- which is the expected outcome, since it evaluates 25 sigma points per step
against a single analytic Jacobian propagation. We therefore find no advantage
to the UKF for this problem and use the MEKF throughout the closed-loop study.
Two caveats matter. The sigma-point spread parameter must be
$O(0.1\text{--}1)$: with the frequently quoted $\alpha = 10^{-3}$ and
$n_x = 12$ one obtains $n_x + \lambda = 1.2\times10^{-5}$, a spread of
$0.0035\sigma$ and a centre weight of $-10^{6}$, at which point the unscented
transform has collapsed onto a finite-difference linearisation and an ``EKF
versus UKF'' comparison compares a filter with itself. And these timings are
on unpinned consumer hardware, so the 3.7\% gap should be read as ``no
meaningful difference'', not as a measurement.
...
global rotation-vector residual in the same configuration yields
...
\begin{figure}[t]
    ...
    \caption{MEKF and UKF tracking errors against time for an identical
...
on a 20~m approach, sharing the same state and control weights, sample time and
thrust limit $u_{max} = 0.1$~m/s$^2$, with the LQR solution as the MPC
terminal cost. The approach corridor is disabled so that thrust saturation is
the only active constraint.

\begin{table}[t]
\caption{LQR against MPC. 20~m approach, $u_{max} = 0.1$~m/s$^2$, $N = 20$.}
\label{tab:control}
\centering
\begin{tabular}{lrr}
\hline
Metric & LQR & MPC \\
\hline
Final range [m]                & 0.0000 & 0.0000 \\
Total $\Delta v$ [m/s]         & 16.83  & 5.00 \\
Max $\|\mathbf{u}\|$ [m/s$^2$] & 0.1014 & 0.1009 \\
Time per step [ms]             & 0.01   & 0.57 \\
\hline
\end{tabular}
\end{table}
...
box-constrained problem is solved first, and the corridor-constrained problem
only when the corridor is violated at that optimum. With the corridor active
throughout, the per-step cost rises by two orders of magnitude and becomes
dominated by a small number of hard early steps, for which a median and 95th
percentile should be reported rather than a mean.
...
\begin{figure}[t]
    ...
    \caption{Range, thrust and cumulative $\Delta v$ for the LQR and MPC
    approaches.}
    \label{fig:lqr_mpc_cmp}
\end{figure}
...
$\sigma_{px} = 1.5$~px keypoint noise, a 0.3~m/s$^2$ thrust limit and a 1~s
control interval. Table~\ref{tab:closedloop} summarises 12 seeds.
...
All 12 seeds dock, in 2.56~m/s of $\Delta v$, with no step reaching the thrust
limit. The absence of saturation is a design requirement rather than an
...
where the target is a few pixels across and every keypoint projects inside
the frame, with the terminal phase, where the target overfills the sensor. The
trial-level rate is accordingly almost constant across the dispersion:
...
\hline
30--50   & 237 & 15.00 & 100.0\% & [98.4,\,100.0] \\
20--30   & 553 & 15.00 & 100.0\% & [99.3,\,100.0] \\
15--20   & 429 & 15.00 & 100.0\% & [99.1,\,100.0] \\
10--15   & 467 & 15.00 & 100.0\% & [99.2,\,100.0] \\
...
level. The trials are \emph{paired}: the same seed sequence generates the
initial position, velocity, tumble rate and attitude at every noise level, so
the level-to-level difference is the effect of the parameter and not of a
fresh draw of initial conditions.
...
falls from $100\%$ to $85\%$ over the same interval. The failures are
therefore not a gradual loss of terminal accuracy: the median trial at 4~px
docks as tightly as the median trial at 0.3~px, while roughly one trial in
seven diverges completely. This is the signature of a threshold, not of drift.
The threshold is the point at which the pose solver's success rate inside
$z_{fill}$ falls low enough that the filter coasts on prediction for the whole
terminal phase, and the accumulated dead-reckoning error exceeds the docking
corridor before the range gate is satisfied.
...
The comparison of pipeline configurations is made over a dispersed Monte Carlo
campaign rather than over repeated runs of one trajectory. Each trial draws an
initial range log-uniformly on $[15, 45]$~m, a bearing uniformly within a
...
sequence, so they are compared trial-by-trial on identical initial conditions.
...
about which subsystem is binding --- which at this noise level is neither. The
configuration comparison becomes discriminating only above 3~px, where
Table~\ref{tab:ablation} shows the perception chain taking over as the
limiting factor.
...
\begin{figure}[t]
    \centering
    \includegraphics[width=\columnwidth]
    {figures/expF_fig8_montecarlo.png}
    \caption{Terminal range, $\Delta v$ and docking success over 100 dispersed
    trials per configuration, paired across configurations. Error bars are
